...
var billingSet = wire.NewSet(
	...
	wire.Bind(new(billingapp.InvoiceResendSender), new(*billingNotificationAdapter)),
	...
	billingapp.NewSendInvoiceEmail,
...
func InitializeApp(cfg config.Config, logger *slog.Logger, pool *pgxpool.Pool) (*gin.Engine, *emailscheduler.Scheduler, error) {
	wire.Build(
		...
		wire.Bind(new(billingapp.SendInvoiceTxManager), new(*transaction.Manager)),
