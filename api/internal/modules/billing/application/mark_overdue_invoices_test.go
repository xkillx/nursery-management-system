...
func (s *stubBillingRepo) GetInvoiceForManagerReviewTx(_ context.Context, _ any, _, _, _ uuid.UUID) (domain.InvoiceReviewRow, bool, error) {
...
func (s *stubBillingRepo) DeleteInvoice(_ context.Context, _ domain.Tx, _, _, _ uuid.UUID) (int64, error) {
	panic("stub")
}
...
func (s *stubBillingRepo) UpdateInvoiceLine(_ context.Context, _ domain.Tx, _, _, _ uuid.UUID, _ string, _ int, _, _ domain.Money, _ []byte) (int64, error) {
	panic("stub")
}
...
func TestMarkOverdue_LondonDateNotUTC(t *testing.T) {
	...
	uc := NewMarkOverdueInvoices(repo, events.NewEventDispatcher(&stubTxMgr{repo: repo}), func() time.Time { return now }, nil)
...
func TestMarkOverdue_LockNotAcquired(t *testing.T) {
	...
	uc := NewMarkOverdueInvoices(repo, events.NewEventDispatcher(&stubTxMgr{repo: repo}), func() time.Time { return time.Now() }, nil)
...
func TestMarkOverdue_IdempotentDoubleRun(t *testing.T) {
	...
	uc := NewMarkOverdueInvoices(repo1, events.NewEventDispatcher(&stubTxMgr{repo: repo1}), func() time.Time { return now }, nil)
	...
	uc2 := NewMarkOverdueInvoices(repo2, events.NewEventDispatcher(&stubTxMgr{repo: repo2}), func() time.Time { return now }, nil)
