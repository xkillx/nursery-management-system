...
func (uc *MarkInactive) Execute(ctx context.Context, actor tenant.ActorContext, childID string, params MarkInactiveParams) (domain.Child, error) {
	...
	err = uc.txm.ExecTx(ctx, func(tx pgx.Tx) error {
		...
		if err := child.Deactivate(domain.ReasonCode(params.ReasonCode), deactivatedAt); err != nil {
			return domainerrors.Validation("Invalid request payload.", "reason_code")
		}
		...
		if err := uc.repo.CloseCurrentRoomAssignment(ctx, tx, actor.TenantID, actor.BranchID, id, deactivatedAt); err != nil {
