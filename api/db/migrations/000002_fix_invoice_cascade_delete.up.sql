-- Fix invoice delete cascade: allow deleting invoice_lines when parent invoice is deleted.

-- 1. Add ON DELETE CASCADE to invoice_lines foreign key
...
-- 2. Update trigger to allow deletion for 'void' status (not just 'draft')
...
BEGIN
    ...
    IF inv_status IS NOT NULL AND inv_status NOT IN ('draft', 'void') THEN
        CASE TG_OP
            WHEN 'INSERT' THEN
