...
|---|---|---|---|
| 1 | Project bootstrap | Initialize Gin API skeleton, `/api/v1` router, env config, local Postgres connection, golang-migrate wiring | API starts and health endpoint responds |
| 2 | Auth base | Login/logout, JWT access (15m), refresh token store (30d), password hash and verify | Test user logs in/out and refreshes |
| 3 | Roles and guards | RBAC middleware for manager/practitioner/parent + tenant/branch scope checks | Protected route rejects wrong role/scope |
| 4 | Core schema | Create tables: users, memberships, children, guardians, links, sessions, audit_logs | Migrations succeed on clean DB |
...
| Day | Focus | Tasks | Done check |
|---|---|---|---|
| 8 | Attendance schema/API | Create event model (`check_in`,`check_out`,`correction`) + endpoints | Single check-in/check-out works |
| 9 | Attendance validations | Prevent duplicate active check-in, enforce correction reason, timezone policy | Duplicate sign-in is blocked |
| 10 | Attendance UI | Practitioner flow: today list, check-in, check-out | Practitioner can run full day flow |
