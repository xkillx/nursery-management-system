...
| FE-21 | ~~Integrate invoice run with `/api/v1` invoice draft generation and issue endpoints.~~ | ~~Invoice API~~ | ~~Incomplete attendance blocks only affected children and appears in exceptions. ✅ Done 2026-06-09 — 419 Angular tests pass, build clean.~~ |
| FE-22 | ~~Build manager invoice list/detail. Include status filters, per-child invoice identity, line items, funded deduction summary, net due, due/overdue status, and immutable issued state.~~ | ~~FE-21~~ | ~~Issued invoices have no direct edit controls. ✅ Done 2026-06-09 — 454 Angular tests pass, build clean.~~ |
| FE-23 | ~~Add lean manager payment/reconciliation UX. Show payment status, payment event detail, retry checkout action where applicable, and basic webhook/payment history if API exists.~~ | ~~Payments API~~ | ~~Manager can understand paid/unpaid/failed status and retry payment flow. ✅ Done 2026-06-09 — 511 Angular tests pass, build clean.~~ |
| API-BILL-01 | **Frontend portion deferred/skipped while the API-BILL-01 backend endpoint remains deferred.** Add minimal issued-invoice adjustment UI after core invoicing works, if API exists. Require reason and show linked adjustment invoice relationship. | API-BILL-01 backend endpoint promoted from deferred | No adjustment entry point appears before invoice is issued; no adjustment UI is built while the API-BILL-01 backend work remains deferred. |
| FE-25 ~~done 2026-06-10~~ | ~~Create parent portal layout. Use same auth/session infrastructure, but only parent-relevant navigation and no staff sidebar/admin template pages.~~ | ~~FE-02~~ | ~~Parent users land in parent portal, not staff dashboard. Standalone ParentPortalLayoutComponent with top nav, user dropdown, theme toggle, no sidebar. Staff sidebar cleaned of parent branch. 518 Angular tests pass.~~ |
| FE-26 ~~done 2026-06-10~~ | ~~Build parent invoices page grouped by child, with urgent unpaid, failed, and overdue invoices surfaced first.~~ | ~~Parent invoice API contract~~ | ~~Parent sees only issued-or-later invoices for linked children. Attention section (overdue, payment_failed, payable issued) before child-grouped history. Payment CTA for payable invoices. Load-more pagination. 579 Angular tests pass, build clean.~~ |
| FE-27 ~~done 2026-06-10~~ | ~~Add parent invoice detail and Stripe Checkout redirect action. Call API to create a fresh checkout session, then redirect browser to hosted Stripe Checkout.~~ | ~~Stripe checkout API~~ | ~~No card fields or custom payment UI exist in Angular. Parent-safe detail with summary, timestamps, calculation, line items. Pay action for payable invoices only. Stripe redirect via hosted Checkout. 579 Angular tests pass, build clean.~~ |
| FE-28 ~~done 2026-06-10~~ | ~~Add payment return/status UX. On return from Stripe, refresh invoice status and optionally poll briefly until paid/failed state is visible.~~ | ~~FE-27~~ | ~~Parent sees clear paid, failed, canceled, or still-processing states. Success return polls every 2s for 20s, resolves to paid/failed/processing. Cancelled return shows canceled copy. Query params cleared with replaceUrl. 595 Angular tests pass, build clean.~~ |
...
| ID | Task | Dependencies | Done check |
|---|---|---|---|
| FE-PM-01 | Build manager child registration/enrolment editor based on `docs/forms/child-application-form.md`. Use sectioned forms for child demographics, medical information, doctor/health visitor, social-services contact, developmental concerns/referrals, dietary requirements, parent/carer details, parental responsibility, emergency contacts, authorised collectors, benefits/funding notes, routines/free-text notes, and office-use fields. **Done.** Includes office-use checklist metadata UI (formerly FE-PM-04 scope). | FE-11; API-PM-01 | Manager can save partial sections, see completion state, and review missing registration requirements without losing existing child/guardian CRUD behavior. |
| FE-PM-02 | [DEFERRED] Parent/guardian digital registration and consent journey. Not part of the manager-assisted registration intake tranche. Manager-Assisted Registration Intake (implemented in `web/src/app/features/staff/pages/manager-registration-intake/`) provides the manager-transcription workflow for physical forms instead. When this item is picked up, reuse registration profile and consent ledger APIs. | Parent portal shell; API-PM-01; API-PM-02 | Parent/guardian can complete required registration and consent fields on mobile and desktop; validation explains missing required fields; submitted consent state is read-only except through an explicit supersede/update flow. |
| FE-PM-03 | [DEFERRED] Add consent review/history UI for managers. Manager-assisted consent record creation and history viewing is already available through the registration intake workflow and the consent ledger API (GET/POST `/children/{child_id}/registration-consents`). A dedicated consent review panel may be added in a future UI pass. | FE-PM-01; API-PM-02 | Manager can quickly inspect current consent before operational decisions and can trace who changed a decision and when. |
| FE-PM-04 | Add registration office-use checklist UI for deposit, application/start/date-left, sessions/days requested, term-time-only status, contract/handbook handoff, Red Book check, birth certificate/passport check, and proof-of-address check. **Folded into FE-PM-01.** Checklist metadata UI implemented within the registration editor; evidence/file-upload work remains for future follow-up. | FE-PM-01; API-PM-03 | Manager can mark document/checklist completion from child enrollment; checklist status feeds the child detail readiness summary. |
| FE-PM-05 | Build room/session planning and booking UI. Include room setup, session templates, child bookings, extra-session requests, capacity warnings, eligibility messages, and links from child detail to booked sessions. | FE-PM-01; API-PM-04 | Manager can plan and adjust booked attendance from a calendar/list workflow; capacity and child-eligibility failures are visible before save; practitioners see only the operational session list needed for attendance. |
| FE-PM-06 | Build ratio safety dashboard and live room checks. Show room-level ratio state, staff assignment, child age-band counts, at-risk sessions, and manager override flow where the API allows it. | FE-PM-05; API-PM-05 | Manager can see unsafe or near-limit sessions before and during the day; practitioner attendance surfaces clear ratio warnings without exposing unnecessary staff or child-sensitive data; override actions require reason and confirmation. |
