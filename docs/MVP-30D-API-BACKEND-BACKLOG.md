...
| API-05 | Add password reset backend: reset request endpoint, token table, token hashing, expiry, set-new-password endpoint, email dispatch through provider abstraction. | API-03, email provider config | User can request reset and set a new password with a valid token; invalid/expired/used tokens return stable errors. |
| API-06 | Add manager invite backend for practitioner and parent roles only: invite create/list/resend/revoke, token hashing, expiry, accept invite, set password, create/activate membership. | API-03, email provider config | Manager can invite non-manager users; manager role is rejected; accepted invite creates login-ready membership in selected tenant/branch. |
| API-07 | Harden existing child/guardian/link/mapping APIs against frontend contract. Confirm manager-only writes, active/default listing behavior, lifecycle reason handling, parent access revocation cascades, and no hard delete endpoints. | Existing people modules | Route tests and smoke checks prove role/scope boundaries and lifecycle idempotency. |
| API-08 | Produce an API contract note for frontend integration covering auth, invite, password reset, child, guardian, mapping, and attendance endpoints. | API-04 to API-07 | `docs/` contains route summary, request/response examples, and known error codes for implemented Week 1 routes. |

## Week 2 - Attendance Hardening and Funding v1

| ID | Task | Dependencies | Done check |
|---|---|---|---|
| API-09 | Verify existing attendance daily list, check-in, check-out, and manager correction endpoints against `CONTEXT.md`. | Existing attendance module | Current endpoints use server time for routine capture, manager-only corrections, required correction reasons, and no routine reason codes. |
| API-10 | Harden attendance validation and billing-readiness queries: duplicate open session, no check-out without open session, overlap detection, future correction guard, enrollment-window guard, incomplete-session discovery by month. | API-09, API-03 | Tests cover duplicate/open/incomplete/overlap/enrollment scenarios and month preflight query returns blocked children. |
| API-11 | Add billing calculation package for attendance-derived minutes. Implement `Europe/London` month boundaries, check-in-month allocation for cross-midnight sessions, incomplete exclusion, and 15-minute round-up per session. | API-10 | Table-driven tests cover same-day, cross-midnight, cross-month, incomplete, corrected, and multiple-session cases. |
| API-12 | Add Funding v1 schema and module: `funding_profiles` with monthly funded-hours allowance per child, manager read/update endpoints, audit events, tenant/branch scope, and validation. | API-03, API-07 | Manager can save/read allowance; practitioner/parent are forbidden; funding update audit event is persisted. |
