...
### Holiday Period
A branch-level date range representing a school holiday within or between academic terms (e.g., "October Half-Term", "Christmas Holiday", "Summer Holiday"). Stored in `holiday_periods` with `start_date`, `end_date`, and `type` (`half_term`, `christmas`, `easter`, `summer`, `other`). Billing code excludes holiday period dates from term-time-only minute calculations. Replaces individual closure days for multi-day school holidays; bank holidays and inset days remain as individual closure days. Holiday periods restrict term-time bookings only — stretched-model children can attend during holiday periods but not on closure days.
